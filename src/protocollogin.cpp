                                     ...
                                     const std::string& token, uint16_t version)
{
	...
	for (uint8_t i = 0; i < size; i++) {
		...
		if (g_config.getBoolean(ConfigManager::ONLINE_OFFLINE_CHARLIST)) {
			output->addByte(getGlobalGame().getPlayerByName(character) ? 1 : 0);
		} else {
			output->addByte(0);
		}
...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	g_dispatcher.addTask(createTask([=, thisPtr = std::static_pointer_cast<ProtocolLogin>(shared_from_this()),
