...
void Npcs::reload()
{
	const std::map<uint32_t, Npc*>& npcs = getGlobalGame().getNpcs();
...
void Npc::addList() { getGlobalGame().addNpc(this); }

void Npc::removeList() { getGlobalGame().removeNpc(this); }
...
void Npc::reload()
{
	...
	getGlobalGame().map.getSpectators(players, getPosition(), true, true);
...
void Npc::doSay(const std::string& text) { getGlobalGame().internalCreatureSay(this, TALKTYPE_SAY, text, false); }
...
void Npc::turnToCreature(Creature* creature)
{
	...
	getGlobalGame().internalCreatureTurn(this, dir);
}
...
int NpcScriptInterface::luaDoSellItem(lua_State* L)
{
	...
	if (it.stackable) {
		while (amount > 0) {
			...
			if (getGlobalGame().internalPlayerAddItem(player, item, canDropOnMap) != RETURNVALUE_NOERROR) {
				delete item;
