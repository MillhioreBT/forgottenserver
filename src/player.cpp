                            ...
                            const Position& oldPos, bool teleport)
{
	...
	if (hasFollowPath && (creature == followCreature || (creature == this && followCreature))) {
		...
		g_dispatcher.addTask(createTask([id = getID()]() { getGlobalGame().updateCreatureWalk(id); }));
	}
...
bool Player::setAttackedCreature(Creature* creature)
{
	...
	if (creature) {
		g_dispatcher.addTask(createTask([id = getID()]() { getGlobalGame().checkCreatureAttack(id); }));
	}
	return true;
}
